...
async function callGenerate(problemType, optimization, seed) {
  if (USE_PROXY) {
    ...
    const url = `${API}/csp/generate?problem_type=${problemType}&optimization=${optimization}${seed!==''?`&seed=${seed}`:''}`;
    const r = await fetch(url);
    return await r.json();
  }
}
...
async function callGrade(payload, answer) {
  ...
  if (USE_PROXY) {
    const r = await fetch("api/proxy_csp_grade.php", {
      method: "POST",
    ...
    const r = await fetch(`${API}/csp/grade`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body
    });
...
async function loadQuestion() {
  try {
    ...
    const optimizationEl = document.getElementById("optimization");
    ...
    const optimization = optimizationEl.value;
    const seed = seedEl.value;

    const q = await callGenerate(problemType, optimization, seed);
    currentPayload = q;
